...
```{r fig.align='center', out.width = '75%'}
## Run this code chunk when you would like to see what the answer to
## Task 02 should look like.
...
```{r fig.align='center', out.width = '75%'}
ggplot(data = penguins %>% filter(!is.na(sex)) %>% count(sex), aes(x = "", y = n, fill = sex)) +
  geom_col() +
  coord_polar("y")
```
...
Create a second histogram of penguin flipper length, and stratify this by species cut. Which penguin species has the longest average flipper length?
